...
if __name__ == "__main__":
    ...
    st.sidebar.write("Becker DE, Reed KL. Local anesthetics: review of pharmacological considerations. Anesth Prog. 2012 Summer;59(2):90-101; quiz 102-3. doi: 10.2344/0003-3006-59.2.90. PMID: 22822998; PMCID: PMC3403589.")
    ...
    if input_answer or check:
        ...
        differential = round(abs(answer - input_answer), 2)
        st.success(f"{answer} mL")
        if differential < 5:
            st.success(f"Off by: {differential} mL")
        elif differential < 10:
            st.warning(f"Off by {differential} mL")
        else:
            st.error(f"Off by: {differential} mL")
